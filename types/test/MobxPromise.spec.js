...
describe('MobxPromise', () => {
    ...
    it('triggers mobx as expected', () => __awaiter(this, void 0, void 0, function* () {
        ...
        let reaction = spy(() => {
            ...
            return { status, result, error, isPending, isError, isComplete };
        });
        ...
        chai_1.assert.isTrue(params.reaction.calledTwice, 'params.reaction() called only twice');
        chai_1.assert.isTrue(params.reaction.firstCall.calledWith(INITIAL), 'params.reaction() called first time with initial value');
        chai_1.assert.isTrue(params.reaction.lastCall.calledWith(value.get()), 'params.reaction() called second time with final value');
        return true;
    }));
});
